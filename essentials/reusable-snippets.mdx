...
description: "Write once, reuse everywhere — keep content in sync across pages"
icon: "recycle"
---
...
<Note>
  Files inside `snippets/` are not rendered as standalone pages — they only appear where imported.
</Note>
...
<Warning>
  Use arrow function syntax (`const Foo = () => ...`). The `function` keyword is not supported in snippet files. Also, MDX syntax (like Mintlify components) doesn't compile inside arrow function bodies — use plain HTML tags there, or use a default export instead.
</Warning>
